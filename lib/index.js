...
try {
	...
	const { default: fetchModule } = await import('node-fetch').catch(() => {});
	fetch = fetchModule;
}
...
program
	...
	.option(
		'-i, --owner-id <id>',
		' Set the Discord bot token. If not specified, use bot owner'
	)
...
const DISCORD_INVITE_BASE = 'https://discord.gg';
...
(async () => {
	try {
		...
		console.log(
			`\u001b[32m✓\u001b[0m Invite created: ${DISCORD_INVITE_BASE}/${invite.code} - Now waiting for the owner to join...`
		);
		...
		ws.on('close', (data) => {
			console.log(
				`\u001b[31m✕\u001b[0m Connection closed: ${JSON.stringify(
					data,
					null,
					'\t'
				)}`
